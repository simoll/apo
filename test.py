...
# some input samples
idx_Dummy = -255 # unused operand position
...
# num_Indices = num_Params + max_Distance
...
# example program
# TODO use tf.placeholder instead
# data = tf.placeholder(data_type(), [None, None, num_features])
...
# program = tf.constant([[[oc_Add, -1, -2], [oc_Sub, 0, -1], [oc_Ret, 1, idx_Dummy]]]) # original program
...
with tf.Session() as sess:
    ...
    # op code translation
    # oc_data = tf.placeholder(data_type(), [None, num_features], name = "statements")
    ...
    # def fetch_operand(idx, outputs):
    #   return tf.cond(idx == idx_Dummy, zeros, tf.cond(idx < 0, tf.gather(param_data, -idx - 1), tf.gather(outputs, idx)))
    ...
    with tf.variable_scope("DAG"): # Recursive Dag Network
      for time_step in range(max_Time):
        ...
        with tf.variable_scope("time_{}".format(time_step)): # Recursive Dag Network
            ...
            # invoke cell
            (cell_output, state) = cell(time_input, state, name="LSTM")
